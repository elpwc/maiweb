...
export const drawOutRing = (ctx: CanvasRenderingContext2D) => {
  ...
  keys.forEach((key) => {
    ...
    // 螺丝孔
    ctx.beginPath();
    ctx.arc(center[0] + keySideDotRtoCenter * cos(key.leftAngle! - keySideDotDistance), center[1] + keySideDotRtoCenter * sin(key.leftAngle! - keySideDotDistance), keySideDotR, 0, 360);
    ctx.closePath();
    ctx.fillStyle = 'gray';
    ctx.fill();

    ctx.beginPath();
    ctx.arc(center[0] + keySideDotRtoCenter * cos(key.rightAngle! + keySideDotDistance), center[1] + keySideDotRtoCenter * sin(key.rightAngle! + keySideDotDistance), keySideDotR, 0, 360);
    ctx.closePath();
    ctx.fillStyle = 'gray';
    ctx.fill();
  });

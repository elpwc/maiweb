...
export const drawOutRing = (values: MaimaiValues, keys: Area[], ctx: CanvasRenderingContext2D) => {
  ...
  keys.forEach(key => {
    ...
    ctx.lineTo(values.center[0] + values.maimaiScreenR * cos(key.leftAngle! - values.keySideLineDistance), values.center[1] + values.maimaiScreenR * sin(key.leftAngle! - values.keySideLineDistance));
    ctx.closePath();
    ctx.strokeStyle = '#555555';
    ctx.stroke();
